<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=DC143C&height=180&section=header&text=MAXAVIER%20JEAN-PHILIPPE&fontSize=42&fontColor=FFD700&animation=twinkling&fontAlignY=32&desc=AI%20Engineer%20•%20Security%20Automation%20•%20Production%20Systems&descSize=18&descAlignY=52"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=25&duration=3000&pause=1000&color=DC143C&center=true&vCenter=true&multiline=true&repeat=true&width=900&height=90&lines=Generative+AI+%2B+Security+Automation+Engineer;I+ship+LangChain+agents+that+triage+SOC+alerts+and+patch+vulnerabilities;Enterprise-ready+AI+with+evals%2C+governance%2C+and+human-in-the-loop+guardrails;Claude+Code+%2B+GitHub+Copilot+velocity+without+sacrificing+safety" alt="Typing SVG" />

<br/>

![Available for Work](https://img.shields.io/badge/Status-Available_for_Consulting-success?style=for-the-badge)

</div>

---

<div align="center">

![American Express](https://img.shields.io/badge/American_Express-2E77BC?style=for-the-badge)
![Truist](https://img.shields.io/badge/Truist-6B2BD9?style=for-the-badge)
![Bank of America](https://img.shields.io/badge/Bank_of_America-C41230?style=for-the-badge)
![PNC Bank](https://img.shields.io/badge/PNC_Bank-F58025?style=for-the-badge)
![UBS](https://img.shields.io/badge/UBS-E60000?style=for-the-badge)

</div>

---

## 🔨 Now

> - Shipping agentic GenAI at **American Express**
> - Building LLM SOC triage + agentic patch automation
> - **Taking 1 new consulting client this quarter**

---

## 🎯 What I Do

I build **production AI systems** and **security automations** in **Python** with **10+ years** across fintech, security, and AI. I'm shipping GenAI at a Fortune 50 bank (**American Express**), and I build agentic workflows that automate SOC triage, vulnerability patching, ServiceNow change requests, and Slack incident response. My delivery style is shaped by tier-1 governance standards from **Amex, Truist, Bank of America, PNC, and UBS**.

---

## 🏢 Experience

| Role | Company | Highlights |
|------|---------|------------|
| **Generative AI Engineer** | American Express (2024–Present) | Python/FastAPI GenAI on GCP · Vertex AI + Gemini + OpenAI · LangChain RAG + agentic workflows · OpenTelemetry for prompt/version/token/latency telemetry |
| **Cybersecurity Engineer** | Truist Bank (2022–2024) | Migrated vulnerability pipelines to AWS ECS Fargate · Unified Qualys + Veracode in ServiceNow · Automated executive reporting and tuned thresholds to cut false positives |
| **Python Developer** | Bank of America (2018–2021) | Secure Python trading services · REST APIs + Angular UI · Repository governance and audit trails |
| **Independent Consulting** | AI & Security (2022–Present) | Agentic patch automation · LLM SOC triage · Detection Engineering CMS · Slack IR bot · ServiceNow change automation · FHIR/HL7 processing on GCP Dataflow |

<div align="center">

> **10+ years** building production Python systems across **fintech, security, and AI**

</div>

---

## 💼 What I Deliver

<table>
<tr>
<td width="50%" valign="top">

### 🤖 LLM-Powered Automation
- Agentic workflows & tool orchestration
- RAG systems with grounded citations
- Evals, confidence gating, safe fallbacks, and human-in-the-loop controls
- OpenTelemetry tracing for prompts, versions, latency, and outcomes

</td>
<td width="50%" valign="top">

### 🛡️ Security Operations
- SOC triage, alert enrichment, and severity-based routing
- On-call escalation and Slack incident response bots
- Detection lifecycle management with analyst handoff
- Prompt injection defense and PII redaction guardrails

</td>
</tr>
<tr>
<td width="50%" valign="top">

### ⚙️ ITSM & Change Automation
- ServiceNow change validation and approval orchestration
- Audit trails and model governance controls
- Governance-ready change workflows for enterprise environments

</td>
<td width="50%" valign="top">

### ☁️ Cloud-Native APIs
- FastAPI production services
- Docker, Kubernetes, and AWS ECS Fargate deployments
- CI/CD with GitHub Actions and Jenkins
- Least-privilege IAM and operational hardening

</td>
</tr>
</table>

---

## 💼 Consulting Packages

| Package | Timeline | Outcome |
|---|---|---|
| **🚀 RAG / Agent Pilot** | 2–4 weeks | Production-grade LangChain workflow with evals, grounded citations, and OpenTelemetry observability |
| **🛡️ SOC Automation Sprint** | 4–6 weeks | LLM-powered triage assistant, Slack IR bot, ServiceNow integration with audit trails |
| **🤝 Fractional AI/Security Engineer** | Monthly retainer | Embedded with your team — ship continuously, governance-ready code, full observability |

*Every engagement includes production-grade code, evals, observability, and clear documentation.*

---

## 🤖 AI-Native Engineering

I build with **Claude Code** and **GitHub Copilot** in the loop — pairing senior judgment with AI velocity. Clients get faster shipping, tighter evals, and fewer regressions, without sacrificing the safety guardrails enterprise teams require.

<div align="center">

![Claude](https://img.shields.io/badge/Claude-2B2B2B?style=for-the-badge)
![GitHub Copilot](https://img.shields.io/badge/GitHub_Copilot-181717?style=for-the-badge&logo=githubcopilot&logoColor=white)

</div>

---

## 🔄 How I Work

<table>
<tr>
<td width="25%" align="center">

### 1️⃣ Discovery
Understand your problem & success criteria

</td>
<td width="25%" align="center">

### 2️⃣ Proposal
Clear deliverables, timeline & pricing

</td>
<td width="25%" align="center">

### 3️⃣ Sprint
Iterative delivery with demos

</td>
<td width="25%" align="center">

### 4️⃣ Handoff
Docs, training & support

</td>
</tr>
</table>

---

## 🛠️ Tech Stack

<div align="center">

<img src="https://skillicons.dev/icons?i=python,fastapi,docker,kubernetes,aws,gcp&theme=dark&perline=6" alt="Core Stack" />

<br/><br/>

![LangChain](https://img.shields.io/badge/LangChain-121212?style=for-the-badge&logo=chainlink&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![Vertex AI](https://img.shields.io/badge/Vertex_AI-4285F4?style=for-the-badge&logo=googlecloud&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini-1A73E8?style=for-the-badge&logo=google&logoColor=white)
![OpenTelemetry](https://img.shields.io/badge/OpenTelemetry-000000?style=for-the-badge&logo=opentelemetry&logoColor=white)
![RAG](https://img.shields.io/badge/RAG-FF6B6B?style=for-the-badge)

</div>

---

## 🚀 Let's Work Together

<div align="center">

🇺🇸 **NYC · ET · Remote · Available for 10–30 hrs/week**

<br/>

[![📅 Book a 20-min intro call](https://img.shields.io/badge/📅_Book_a_20--min_intro_call-32CD32?style=for-the-badge)](https://calendly.com/maxjeanphilippe/30min)
[![Email Me](https://img.shields.io/badge/✉️_Email_Me-maxjeanphilippe%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:maxjeanphilippe@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/lumen-maximus)
[![GitHub](https://img.shields.io/badge/GitHub-lumen--maximus-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/lumen-maximus)

<!-- 🚧 Freelance platforms coming soon!  -->
<!-- [![Upwork](https://img.shields.io/badge/Upwork-Hire_Me-6fda44?style=for-the-badge&logo=upwork&logoColor=white)](https://www.upwork.com/freelancers/~YOUR_ID) -->
<!-- [![Fiverr](https://img.shields.io/badge/Fiverr-Hire_Me-1DBF73?style=for-the-badge&logo=fiverr&logoColor=white)](https://www.fiverr.com/YOUR_USERNAME) -->

</div>

---

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=lumen-maximus&color=6366f1&style=for-the-badge)

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=DC143C&height=120&section=footer"/>
